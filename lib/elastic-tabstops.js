...
function disable(editor) {
	...
	if (editor._cursorChanged) {
		editor._cursorChanged.dispose()
		delete editor._cursorChanged
	}
...
function enable(editor) {
	...
	const c = editorElement.component
	// monkeyPatch(c, 'update')
	// monkeyPatch(c, 'updateSync')
	// monkeyPatch(c, 'scheduleUpdate')
	// monkeyPatch(c, 'updateSyncBeforeMeasuringContent')
	// monkeyPatch(c, 'measureContentDuringUpdateSync')
	// monkeyPatch(c, 'updateSyncAfterMeasuringContent')
	// monkeyPatch(c, 'render')
	// monkeyPatch(c, 'renderContent')
	// monkeyPatch(c, 'renderLineTiles')
	...
	function createTabstops(containerElement) {
		...
		let first = lines[0]
		let least = first && +first.dataset.screenRow

		for (const line of lines) {
			builder.line(line.dataset.screenRow - least)
...
function monkeyPatch(object, method, patch) {
	if (object[method] == null) {
		console.warn('no method', method, 'on target', object)
		return {
			dispose() {}
		}
	}
	const old = object[method]
	object[method] = incept(old, patch, object, method)
	return {
		dispose() {
			delete object[method]
			if (object[method] !== old) object[method] = old
		}
	}
}
...
function incept(
	...
	const f = wrap ? wrap(origin, object, method) : origin
	return function (...args) {
		if (before) {
			const a = before.call(this, ...args)
			if (a !== undefined) {
		...
		try {
			...
			if (after) {
				const r = after.call(this, result)
				if (r !== undefined) return r === undef ? undefined : r
			}
			return result
		} catch (e) {
			if (error) {
				const r = error.call(this, e)
				if (r !== undefined) return r === undef ? undefined : r
			}
			throw e
		}
