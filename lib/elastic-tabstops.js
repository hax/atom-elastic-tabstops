...
function observeEditor(editor) {
	...
	function update(containerElement) {
		...
		for (const block of cursorBlocks) align(block.cells)
		// for (const block of tabstops.columnBlocks) align(block.cells)
		console.timeEnd('align blocks at cursors')

		// TODO: try to fix cursor positions by force buffer change?
	...
	function align(cells) {
		...
		// Unify width of tabs
		for (const {tab} of cells) tab.style.width = '1ch'
		...
		for (const {leadingSpace, tailingSpace, tab} of cells) {
